   ...
   items.forEach((item) => {
      ...
      item.addEventListener("click", function () {
         ...
         if (!selectedInSameMenu) {
            ...
            document
               .querySelector(`.filters__item-select[data-menu-id='${selectedItems[selectedId]["menuId"]}']`)
               .remove();
            selectedItems.splice(selectedId, 1);
   ...
   if (btn) {
      ...
      window.addEventListener("resize", () => {
         if (document.documentElement.clientWidth > 1024) {
            ...
            if (document.querySelector("body").classList.contains("filters-opened"))
               document.querySelector("body").classList.remove("filters-opened");
         }
      });
   }
   ...
   if (items) {
      items.forEach(function (item) {
         ...
         btn.addEventListener("click", function () {
            if (document.documentElement.clientWidth <= 1024) {
               const correctItem = document.querySelector(
                  `.filters__item-holder[data-accordion-filters="${this.dataset.accordionFilters}"]`
               );
               const correctContent = correctItem.querySelector(".filters__menu");
               const correctHeader = correctItem.querySelector(".filters__item");
      ...
      window.addEventListener("resize", () => {
         if (document.documentElement.clientWidth > 1024) {
            items.forEach((item) => {
               ...
               if (item.querySelector(".filters__item").classList.contains("selected"))
                  item.querySelector(".filters__item").classList.remove("selected");
            });
         }
      });
   }
